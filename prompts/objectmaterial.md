...
When determining the initial temperature, the object name should be considered as well when applicable (e.g. a temperature is given in the object name). Consider the effects of nearby objects and whether they would have heated up or cooled the object when the scene begins (e.g. a pot above an open flame would already be very hot). Additionally, consider whether the object is indoors or outdoors when determining temperature (e.g. a chair outside in a cold environment may be cold, but if it was inside a climate controlled interior, would be the temperature of that interior). Justify this inference and provide explanation why specifically this temperature and not similar temperatures (i.e. why degrees 20 Celsius as opposed to 23 degrees Celsius)
Give these properties in decimal format. If it is a whole number, add a trailing 0 (i.e. 1.0 rather than 1)
Additionally, determine whether or not the object behaves like a heat source(e.g. fire will behave as a heat source, but air conditioning will not even though it would generate heat). If so, estimate its heat generation rate in joules per second and if it is initially on at the beginning of the scene. If you determined the object cannot generate heat, or shouldn't act as a heat source, leave heat generation rate as 0.
